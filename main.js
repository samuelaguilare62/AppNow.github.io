...
function injectCodeIntoCards() {
  ...
  var codeToInject = `
        ...
        <div id="popupShare" class="popupShare">
          ...
          <button onclick="generateImage(this.parentElement)">Descargar Imagen</button>
...
function generateImage() {
  // Obtén la tarjeta y el contenido de title-card
  var card = document.querySelector(".card");
  var titleCard = card.querySelector(".title-card").textContent.trim();

  // Excluye los elementos que no deseas incluir
  var excludedElements = [
    document.getElementById("popupShare"),
    document.querySelector(".button-download"),
    document.querySelector(".btn-info")
  ];

  // Almacena los estilos originales de los elementos excluidos
  var originalStyles = [];
  ...
  // Agrega un elemento de texto con sombra
  var textElement = document.createElement("div");
  textElement.textContent = "WebAppiFy";
  ...
  textElement.style.textShadow = "2px 2px 4px #000000"; // Ajusta según tus preferencias

  card.appendChild(textElement);

  // Utiliza html2canvas con las exclusiones y la adición de texto
  html2canvas(card, {
    ...
    // Restaura la visibilidad y los estilos originales de los elementos excluidos
    originalStyles.forEach(function(style) {
      style.element.style.display = style.display;
    });

    // Elimina el elemento de texto añadido
    card.removeChild(textElement);

    // Crea un enlace de descarga para la imagen generada
    var downloadLink = document.createElement("a");
    downloadLink.href = canvas.toDataURL("image/png");
    downloadLink.download = titleCard + "_image.png";
