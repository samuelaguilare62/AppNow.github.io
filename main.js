...
// Obtener el nombre de la aplicación desde el elemento con clase 'title-card'
const appNameElement = document.querySelector('.title-card');
const appName = appNameElement.innerText;

// Verificar si se encontró el nombre de la aplicación
if (appName) {
    // Cambiar el contenido de la tarjeta basado en el nombre de la aplicación
    // (esto es opcional, dependiendo de tus necesidades)
    // document.querySelector('.title-card').innerText = appName;

    // Modificar el enlace de la página con el parámetro appName
    const pageLink = 'https://webappifly.netlify.app/apps';
    const newPageURL = `${pageLink}?appName=${appName}`;
    
    // Asignar la nueva URL al enlace
    window.history.replaceState({}, document.title, newPageURL);

    // Mostrar la tarjeta (esto es opcional y puede que necesites ajustarlo dependiendo de tu implementación)
    document.querySelector('.card').style.display = 'block';
} else {
    console.log('Nombre de la aplicación no encontrado en el elemento con clase "title-card".');
}
